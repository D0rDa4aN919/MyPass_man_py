...
#         exit()
# if checker == 1:
#     with open("First_Time.py", "r") as file:
#         text = file.read().replace("checker = 1", "checker = 0")
#     with open("First_Time.py", "w") as file:
#         file.write(text)
# ---------------------------- VARIABLES ------------------------------- #
...
# # can be change by script or manually
EMAIL = "dor@gmail.com"
...
# Deserialize the private key with the password
private_key = serialization.load_pem_private_key(
    private_key_pem,
    password=PASS,
    backend=default_backend()
...
def main():
    ...
    window.config(bg=BG, pady=20, padx=20)
...
def read_file():
    ...
    try:
        ...
        for index, row in enumerate(data):
            ...
            # Format the password column value
...
def getting_the_amount():
    ...
    window1.config(padx=50, pady=50, bg=BG)
